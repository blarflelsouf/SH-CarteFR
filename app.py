...
# --------- ISOCHRONE MAPBOX ---------
@st.cache_data
def get_mapbox_isochrone(lon, lat, minutes, mode="driving"):
    ...
    if r.status_code != 200:
        st.error(f"Erreur API Mapbox : {r.status_code} - {r.text}")
        st.stop()
    data = r.json()
    coords = data['features'][0]['geometry']['coordinates'][0]
    # Mapbox: [lon, lat] → Folium: [lat, lon]
...
def _agglos(df_temp, n, mode="km"):
    ...
    agglo = (
        ...
        .apply(lambda g: pd.Series({
            ...
            'Distance': g.loc[g['population'].idxmax()]['distance'] if "distance" in g else None,
...
def villes_dans_rayon_km(df_clean, coord_depart, rayon):
    ...
    df_all_in_radius['distance'] = df_all_in_radius.apply(
        lambda row: geodesic(coord_depart, (row['latitude_mairie'], row['longitude_mairie'])).km, axis=1)
    return df_all_in_radius[df_all_in_radius['distance'] <= rayon].reset_index(drop=True)
...
def gd_villes_dans_rayon_km(df_clean, coord_depart, rayon, min_pop, n):
    ...
    df_temp['distance'] = df_temp.apply(
        lambda row: geodesic(coord_depart, (row['latitude_mairie'], row['longitude_mairie'])).km, axis=1)
    df_temp = df_temp[df_temp['distance'] <= rayon].reset_index(drop=True)
...
def villes_dans_isochrone(df_clean, polygone_isochrone):
    ...
    df_temp['in_isochrone'] = df_temp.apply(
        lambda row: polygone_isochrone.contains(Point(row['latitude_mairie'], row['longitude_mairie'])), axis=1)
    return df_temp[df_temp['in_isochrone']].reset_index(drop=True)
...
def gd_villes_dans_isochrone(df_clean, min_pop, n, polygone_isochrone):
    ...
    df_temp['distance'] = None  # pas de km en isochrone
...
if adresse:
    ...
    if mode_recherche == "Rayon (km)":
        ...
        # 1. Récupérer polygone isochrone Mapbox
        polygone_recherche = get_mapbox_isochrone(lon, lat, temps_min)
        # 2. Villes dans l'isochrone
        df_all_in_radius = villes_dans_isochrone(df_clean, polygone_recherche)
        # 3. Agglos principales
    ...
    df_stats = pd.DataFrame({
        "Indicateur": [
            "Nombre total de villes dans le périmètre",
            "Population totale dans le périmètre",
            "Population totale des grandes villes"],
    ...
    if mode_recherche == "Rayon (km)":
        ...
        # Polygone Mapbox
        folium.Polygon(
            locations=[(lat, lon) for lon, lat in get_mapbox_isochrone(lon, lat, temps_min).exterior.coords],
            color="purple", fill=True, fill_opacity=0.15,
            popup=f"{temps_min} min autour de {adresse}"
        ).add_to(m)
    # Heatmap
    ...
    mode_color = "km" if mode_recherche == "Rayon (km)" else "min"
    for _, row in df_filtre.iterrows():
        couleur = couleur_par_distance(row['Distance'] if row['Distance'] is not None else 0, mode=mode_color)
        folium.Marker(
            location=[row['Latitude'], row['Longitude']],
            popup=f"{row['Ville']} ({row['Distance'] if row['Distance'] is not None else '-'} {'km' if mode_color=='km' else 'min'}, {row['Population']} hab)",
            icon=folium.Icon(color=couleur)
        ).add_to(m)

    st.markdown(
        f"### Carte des {len(df_filtre)} plus grandes agglomérations autour de {adresse}"
    )
    st_data = st_folium(m, width=900, height=600)

    # Tableau HTML coloré
    df_display = df_filtre[['Ville', 'Distance', 'Population', 'Région']].copy()
    if mode_color == "km":
        df_display["Distance"] = df_display["Distance"].apply(lambda x: f"{x:.2f}".replace(".", ",") if x is not None else "-")
        distance_col = "Distance (en km)"
    else:
        df_display["Distance"] = "-"
        distance_col = "Distance (en min)"
    df_display["Population"] = df_display["Population"].apply(lambda x: f"{int(x):,}".replace(",", "."))
    rows = []
    for _, row in df_display.iterrows():
        try:
            dist_val = float(row["Distance"].replace(",", ".")) if row["Distance"] != "-" else None
            couleur = couleur_par_distance(dist_val, mode=mode_color)
        except:
            couleur = "gray"
        ville_html = f'<span style="color:{couleur}; font-weight:bold">{row["Ville"]}</span>'
        rows.append(f"<tr><td>{ville_html}</td><td>{row['Distance']} {distance_col.split('(')[-1][:-1]}</td><td>{row['Population']}</td><td>{row['Région']}</td></tr>")
    table_html = f"""
    ...
    <tr style="background-color:#223366; color:white;">
        ...
        <th style="padding:8px; border:1px solid #AAA;">{distance_col}</th>
